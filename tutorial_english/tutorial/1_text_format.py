...
class WriteText(Scene): 
    def construct(self): 
        text = TextMobject("Praktischerweise hab ich mir jetzt\\\\ die Software vom 3Blue1Brown geholt.")
        self.play(Write(text))
        self.wait(3)

class AddText(Scene): 
    def construct(self): 
        text = TextMobject("quick maths")
        self.add(text)
        self.wait(3)

class Formula(Scene): 
    def construct(self): 
        formula = TexMobject("\int_0^{\infty}\\text{Bier}^2 dt")
